...
async function init() {
	...
	if (window.ethereum) {
		try {
			...
			console.log("Current network id is: ", netId);
			if (netId != dictionaryContract.getNetwork().id) {
				ui.displayToastMessages("error", 0, true, `Please switch to ${dictionaryContract.getNetwork().name} network and reload`);
				return;
			}
	...
	if (isExists) {
		...
		dictionaryContract.addPost(newWord, newPost)
			.then(async transaction => {
				...
				ui.renderPosts(newWord, postsArray);
				ui.displayToastMessages("success", 2, true, `Post added, check the transaction at <a target="_blank" href="${dictionaryContract.getNetwork().etherscanBaseUrl}/tx/${transaction.transactionHash}">etherscan</a>`);
...
async function vote(button) {
	...
	dictionaryContract.votePost(word, postId, buttonType)
		.then(async transaction => {
			...
			const postsArray = await dictionaryContract.getPostsByWord(word);
			ui.renderPosts(word, postsArray);
			ui.displayToastMessages("success", 2, true, `Vote saved, check the transaction at <a target="_blank" href="${dictionaryContract.getNetwork().etherscanBaseUrl}/tx/${transaction.transactionHash}">etherscan</a>`);
